...
if(empty($pertanyaan) || empty($rumusan_kebutuhan) || empty($bidang_layanan) || empty($strategi_layanan) || empty($materi_layanan) ||
  ...
  empty($evaluasi) || empty($target_kelas) || $jumlah==0){
    $isValid = 0;
    $isPesan = "Silahkan lengkapi data !!!";
    $json = array("isValid" => 0, "isPesan" => $isPesan);
}else{
    $q1 = "UPDATE $db.$table_pertanyaan SET kelas='$data_kelas', pertanyaan='$pertanyaan', rumusan_kebutuhan='$rumusan_kebutuhan',
        rumusan_tujuan='$rumusan_tujuan', bidang_layanan='$bidang_layanan', tujuan_layanan='$tujuan_layanan', komponen_layanan='$komponen_layanan',
        strategi_layanan='$strategi_layanan', materi_layanan='$materi_layanan', metode_layanan='$id_metode_layanan', media_layanan='$id_media_layanan',
        evaluasi='$evaluasi', updated_date=now(), updated_by='$created_by' WHERE id='$id'";
