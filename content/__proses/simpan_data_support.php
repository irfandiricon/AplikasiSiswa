...
$qc = "SELECT COUNT(*) as total FROM $db.$table_support";
$ex_qc = mysqli_query($con, $qc);
$row = mysqli_fetch_assoc($ex_qc);
$total = $row['total'];

if($total == 0){
    $q = "INSERT INTO $db.$table_support (isi, created_date, created_by) values ('$keterangan',now(),'$created_by')";
}else{
    $q = "UPDATE $db.$table_support SET isi='$isi', updated_date=now(), updated_by='$created_by'";
}
